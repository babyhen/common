...
public class ConcurrentExecutor {
    ...
    public <V> ConcurrentExecuteResult<V> execute(Collection<? extends AbsConcurrentCall<V>> calls) {
     ...
     */
    public <V> ConcurrentExecuteResult<V> execute(Collection<? extends AbsConcurrentCall<V>> calls, long timeOut) {
        log.debug("开始多线程任务，子任务数量:{},time out is {}", calls.size(), timeOut);
        if (timeOut < 0) {
        ...
        for (AbsConcurrentCall<V> call : calls) {
            call.setCdl(cdl);
            Future<V> future = executor.submit(call);
            futures.add(future);
        }
        try {
            log.debug("wait sub thread finish...");
            ...
            log.debug("all sub thread finish...begin main thread");
            ...
            log.warn("error happends on main thread,{}", e);
            throw new RuntimeException(e);
        }

    }

    public <V> ConcurrentExecuteResult<V> executeBatch(Collection<? extends AbsConcurrentCall<V>> calls, int batchSize) {
    ...
    public <V> ConcurrentExecuteResult<V> executeBatch(Collection<? extends AbsConcurrentCall<V>> calls, int batchSize,
                                                       long timeOut) {
        log.debug("begin concurrent calls，all size:{},batch size:{}", calls.size(), batchSize);
        if (calls == null || calls.size() == 0) {
        ...
        List<List<? extends AbsConcurrentCall<V>>> deviceCalls = CollectionConventer.devide(calls, batchSize);
        for (List<? extends AbsConcurrentCall<V>> batch : deviceCalls) {
